...
Лёгкое Docker‑решение: приложение, PostgreSQL и PGWEB запущены в контейнерах.  
PGWEB доступен на `/pgweb` (логин/пароль в `.env`).

**Ключевые фичи:**
- Контекстный NLP + XLM‑RoBERTa скоринг по 34 компетенциям  
- Два интерфейса: самооценка кандидата и дашборд HR  
- Авто‑рассылка писем через Gmail API  
- MLOps‑готовность: кеш, CI/CD, мониторинг 
