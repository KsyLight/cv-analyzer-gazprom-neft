...
if uploaded_file:
    ...
    try:
        ...
        with st.spinner("⏳ Извлечение текста..."):
            ...
            # ─── Новый блок сбора GitHub‑текста ────────────────────────────────────
            ...
            if gh_links:
                ...
                for link in gh_links:
                    ...
                    try:
                        # Берём именно тот текст, что передаётся модели
        ...
        tab1, tab2, tab3, tab4 = st.tabs([
            "Оценка грейдов", "Профессии", "Рекомендации", "Резюме"
        ])
        ...
        with tab2:
            if "user_grades" not in st.session_state:
                st.warning("⚠️ Сначала заполните грейды во вкладке 'Оценка грейдов'")
                st.stop()
            ...
            with col2:
                ...
                descriptions = {
                    "Аналитик данных (Data scientist, ML engineer)": "Специалист, который работает с данными компании, анализирует их и разрабатывает решения на основе ИИ. • Определяет метод ML и адаптирует его к задаче • Разрабатывает признаки • Строит пайплайн • Ведёт документацию",
        ...
        with tab4:
            ...
            with st.expander("📝 Текст из файла резюме", expanded=False):
                st.text_area("Текст из резюме", base_text, height=500, disabled=True)
            ...
            with st.expander("🧑‍💻 Текст, собранный с GitHub", expanded=False):
                ...
                if github_text_final:
                    st.text_area("GitHub-текст", github_text_final, height=500, disabled=True)
                else:
