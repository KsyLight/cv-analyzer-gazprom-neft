...
if uploaded_file:
    ...
    try:
        with st.spinner("⏳ Извлечение текста..."):
            ...
            full_text = preprocess_text(base_text + " " + st.session_state.github_text_raw)
        ...
        with col2:
            ...
            sorted_percentages = sorted(percentages, key=lambda x: -x[1])
...
Работает с системами хранения и анализа данных, обеспечивает их эффективное функционирование, а также поддержку систем версионирования данных."""
        ...
        github_text_to_show = st.session_state.get("github_text_raw", "")
        if github_text_to_show.strip():
            with st.expander("🧑‍💻 Текст, собранный с GitHub"):
                st.text(github_text_to_show)
        else:
